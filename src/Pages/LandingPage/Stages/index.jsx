...
export default function Stages() {
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-4 relative  justify-between flex-nowrap wrapcontainer">
        {stages.map((stage, index) => (
          <div
            key={index}
            className="stage-container inline-block md:flex flex-col items-center mx-2 md:mx-0 first:ml-0 last:mr-0 mb-6"
          >
                  ...
                  return (
                    <div key={itemIndex} className="flex gap-2 w-full">
                      {item.items.map((groupItem, groupIndex) => (
                        <div
                          key={groupIndex}
                          className="flex items-center justify-center h-14 md:h-[40px] px-3 py-2 text-white rounded-[40px] border border-[#B946DF] text-noWrap text-[13.5px]"
                        >
                          {groupItem.text}
                        </div>
                      ))}
